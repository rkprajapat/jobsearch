...
try:
    ...
    print(
        "inputs.json not found. Please make sure it exists in the project_data directory."
    )
    sys.exit(1)
...
CLUSTERING_VERSIONED_OUTPUT: bool = CLUSTERING_CONFIG.get(
...
_data_dir_raw = os.getenv("DATA_DIR_PATH")
if not _data_dir_raw:
    print("Data directory not configured. Set environment variable DATA_DIR_PATH.")
    sys.exit(1)

PROJECT_DATA_DIR: Path = Path(_data_dir_raw).resolve()
