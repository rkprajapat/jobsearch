...
async def login_linkedin(
    page: Page, doms: dict, credentials: dict, actions: HumanActions
) -> None:
    ...
    if "linkedin.com/login" not in page.url:
        print(
            f"Already authenticated (redirected to {page.url}) - skipping login form."
        )
        return
    ...
    try:
        await page.wait_for_selector(
            doms["post_login_indicator"], timeout=LOGIN_WAIT_SECONDS * 1_000
        )
        print("Login confirmed.")
    except Exception:
        print(
            f"No 2FA/CAPTCHA challenge detected within {LOGIN_WAIT_SECONDS}s - proceeding with search."
        )
