...
from agents.resume_builder.section_builder import build_sections
from models.opportunity import Opportunity, load_opportunities
from ui.application_state import ApplicationStateManager
from ui.constants import BUTTON_PROPS, CLASSES, CONTENT, LAYOUT, SWITCH_PROPS
...
async def _process_resume_sections():
    spinner = create_overlay_spinner("Processing resume sections...")
    try:
        success = await run.cpu_bound(build_sections)
        if success:
            ui.notify("Resume sections built successfully.")
        else:
            ui.notify(
                "Failed to build some resume sections. Check logs for details.",
                type="negative",
            )
    except Exception as e:
        ui.notify(f"Error processing resume sections: {e}", type="negative")
        traceback.print_exc()
    finally:
        spinner.delete()
...
def _render_summary(state_manager: ApplicationStateManager) -> dict:
    ...
    """
    with ui.row().classes("gap-6 mb-4 w-full"):
        ...
        # Add resume update button
        ui.space().classes("grow")
        ui.button(icon="document_scanner", on_click=_process_resume_sections).props(
            "dense flat"
        ).classes("self-end")
...
async def _render_opportunity_card(
    opp: Opportunity, state_manager: ApplicationStateManager
) -> None:
    ...
    async def on_relevant_change(event, current_opp=opp):
        ...
        success = await state_manager.update_relevant_status(
            current_opp, bool(event.value)
        )
        if not success:
    ...
    async def on_applied_change(event, current_opp=opp):
        ...
        success = await state_manager.update_applied_status(
            current_opp, bool(event.value)
        )
        if not success:
            ui.notify(CONTENT["error_save_failed"], type="negative")
...
async def _prepare_application(opp: Opportunity) -> None:
    ...
    try:
        ...
        await run.cpu_bound(resume_agent.prepare_resume, opp.job_description)
...
def _render_description(job_description: str) -> None:
    ...
    with ui.column().classes(LAYOUT["description_column"]):
        desc_label = ui.markdown(description_toggle.get_current_text()).classes(
            CLASSES["card_meta"]
        )
        ...
        if len(job_description) > int(CONTENT["description_preview_length"]):
            toggle_button = (
                ui.button(description_toggle.get_button_label())
                .props(BUTTON_PROPS["toggle"])
                .classes(CLASSES["toggle_button"])
            )
            toggle_button.on_click(
                lambda _, btn=toggle_button: _toggle_description(
                    desc_label, btn, description_toggle
                )
            )
